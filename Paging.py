...
class Main_UI(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self):
        ...
        start_new_thread(self.udpLogReturn,
                         (LocalID, 'info', 'high', 'Paging Control -- 시스템이 시작되었습니다.'))
        # UI Start
        self.show()
        # Button State Reload
        sleep(1)
        ...
        if state == 1:
            ...
            if self.em_Popup == False:
                ...
                if self.emdialog.exec_():
                    self.em_Popup = False
                    start_new_thread(
                        self.udpLogReturn, (LocalID, 'info', 'mid', 'Paging Control -- 지점에서 비상 상황을 확인 하였습니다.'))
        elif state == 0:
    ...
    def bssReoot_function(self):
        ...
        if self.qdialog.exec_():
            ...
            try:
                ...
                self.udpLogReturn(
                    LocalID, 'warning', 'high', 'Paging Control -- BSS DSP가 Paging Control에 의해서 재부팅 되었습니다.')
            except:
                self.udpLogReturn(
                    LocalID, 'error', 'high', 'Paging Control -- BSS장비와의 통신에 문제가 발생했습니다.')
    ...
    def bssLocalMic_Btn(self):
        ...
        try:
            ...
            if self.setFunctionButton1.isChecked():
                self.udpLogReturn(
                    LocalID, 'info', 'high', 'Paging Control -- 지점 마이크 사용이 시작 되었습니다.')
                self.statusBar().showMessage('지점 마이크 사용이 시작 되었습니다.', 5000)
            else:
                self.udpLogReturn(
                    LocalID, 'info', 'high', 'Paging Control -- 지점 마이크 사용이 종료 되었습니다.')
    ...
    def bssFuction_Btn(self):
        ...
        try:
            ...
            if self.setFunctionButton1.isChecked():
                self.udpLogReturn(
                    LocalID, 'info', 'mid', 'Paging Control -- 지점 이벤트 기능이 활성화 되었습니다.')
                self.statusBar().showMessage('지점 이벤트 기능이 활성화 되었습니다.', 5000)
            else:
                self.udpLogReturn(
                    LocalID, 'info', 'mid', 'Paging Control -- 지점 이벤트 기능이 종료 되었습니다.')
    ...
    def buttonNameSave_(self):
        ...
        start_new_thread(self.udpLogReturn,
                         (LocalID, 'info', 'mid', 'Paging Control -- 릴레이 버튼 이름이 변경되었습니다.'))
                ...
                with open(self.btn_State_FileName[group], 'w') as file:
                    for i in range(24):
                        if self.btn_Ui[group][i].isChecked():
                            ...
                            file.write('0\n')
            start_new_thread(self.udpLogReturn,
                             (LocalID, 'info', 'mid', 'Paging Control -- 버튼 상태값이 저장되었습니다.'))
            self.statusBar().showMessage('버튼 상태값이 저장되었습니다.', 5000)
        except:
            self.statusBar().showMessage('버튼 상태값 저장이 되지 않았습니다.', 5000)
    ...
    def serverIpAddressSave(self):
        ...
        if self.qdialog.exec_():
            try:
                ...
                start_new_thread(
                    self.udpLogReturn, (LocalID, 'warning', 'high', 'Paging Control -- IP가 변경되었습니다. {}'.format(' '.join(self.tcpServerAddress)),))
    ...
    def serverConnectClient(self, state):
        if state == True:
            ...
            start_new_thread(self.udpLogReturn,
                             (LocalID, 'info', 'low', '네트워크 클라이언트가 연결 되었습니다.'))
            ...
            self.tcpServerNetworkStatus = False
            start_new_thread(self.udpLogReturn(
                LocalID, 'info', 'low', '네트워크 클라이언트가 연결이 해제 되었습니다.'))
    ...
    def server_data_parcing(self, data):
        try:
            ...
            start_new_thread(self.udpLogReturn, (LocalID, 'warning', 'low', '네트워크 명령이 잘못되었습니다.'))
